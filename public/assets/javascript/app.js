...
//Display data from volunteer match stored in db
//========================================================================================
// grab the web development volunteer opportunities as a json
//var WebVol = require('../models/WebVol');

$.getJSON('/voldev', function(WebVol) {
    // for each one
    for (var i = 0; i<WebVol.length; i++){
        // display the apropos information on the page
        $('#webvol').append('<p data-id="' + WebVol[i]._id + '">'+ WebVol[i].title + '<br />'+ WebVol[i].link + '</p>');
    }
});
...
$(document).on('click', 'p', function(){
    ...
    // with that done, add the note information to the page
        .done(function( WebVol ) {
            console.log(WebVol);
            // the title of the webvol
            $('#notes').append('<h2>' + WebVol.title + '</h2>');
            // an input to enter a new title
            $('#notes').append('<input id="titleinput" name="title" >');
            // a textarea to add a new note body
            $('#notes').append('<textarea id="bodyinput" name="body"></textarea>');
            // a button to submit a new note, with the id of the webvol saved to it
            $('#notes').append('<button data-id="' + WebVol._id + '" id="savenote">Save Note</button>');

            // if there's a note in the webvol
            if(WebVol.note){
                // place the title of the note in the title input
                $('#titleinput').val(WebVol.note.title);
                // place the body of the note in the body textarea
                $('#bodyinput').val(WebVol.note.body);
            }
        });
});
